...
Limits are 10 names per client address per hour, one change per name per 10 seconds, and 100 active names in the pilot. `update_dns_name` moves the address, never the expiry. `delete_dns_name` removes the name. The primary name server drops it at once and the secondary follows by replication. A disconnected secondary may keep answering until it reconnects or the name reaches its original expiry. A resolver that already holds the answer keeps it for the rest of its TTL.
